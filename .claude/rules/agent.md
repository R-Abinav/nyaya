...
- **Commit-reveal.** Commit `keccak256(abi.encode(caseId, juror, ruling, confidence, salt))` before the commit deadline, then reveal after it. Persist the ruling, confidence and salt durably before sending the commit. A lost salt means no reveal, and no reveal is slashed as incorrect.
