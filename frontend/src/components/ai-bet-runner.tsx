...
function AgentPanel({ agent }: { agent: AgentState }) {
  return <div className="min-h-72 rounded-2xl border border-line bg-canvas p-4"><div className="flex items-center justify-between gap-2"><h3 className="font-bold">{agent.name}</h3><Badge tone={agent.error ? 'danger' : agent.completed ? 'success' : 'warning'}>{agent.error ? 'Failed' : agent.completed ? 'Completed' : agent.status}</Badge></div><div className="mt-4 max-h-72 space-y-3 overflow-y-auto pr-1">{agent.events.length === 0 && <p className="text-sm text-muted-foreground">Waiting for the agent to start…</p>}{agent.events.map((event, index) => <EventRow event={event} key={`${event.timestamp}-${index}`} />)}</div>{agent.error && <p className="mt-4 text-sm text-danger">{agent.error}</p>}</div>;
}
...
function EventRow({ event }: { event: AgentEvent }) {
  ...
  return <div className="border-l-2 border-primary/30 pl-3 text-sm"><p className="font-semibold">{event.type === 'tool_call_started' ? `Calling ${tool}` : event.type.replace(/_/g, ' ')}</p>{tool && <p className="mt-1 text-xs text-muted-foreground">Tool: {tool} · {String(event.data.status ?? 'running')}</p>}{event.type === 'tool_call_completed' && result && <p className="mt-1 text-xs text-muted-foreground">{result.option?.label ? `Selected: ${result.option.label} · ` : ''}{result.amountCents ? `Bet: $${(result.amountCents / 100).toFixed(2)} · ` : ''}{result.payment?.approved ? 'Payment approved' : ''}</p>}{explanation && <p className="mt-2 leading-6 text-muted-foreground">{explanation}</p>}</div>;
}
