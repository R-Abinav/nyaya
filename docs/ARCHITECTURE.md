...
- A juror that withdraws evidence money for a case but never commits has that spend recorded the same way, as a third, separately labelled cause: net `−x_i` on capital `x_i`. **This is by definition a bug signal, not a legitimate agent decision.** Agents always commit after spending, and they express low confidence with a small stake, never by abstaining. So spend with no commitment can only mean a crash, a missed deadline, or a failed transaction. Settlement emits a dedicated `SpentWithoutCommitting` event for it, on top of the normal accounting, so it can trigger an alert. The money left its treasury, so leaving it out would let the tracked return drift from the real balance. Evidence withdrawals close at the commit deadline.
